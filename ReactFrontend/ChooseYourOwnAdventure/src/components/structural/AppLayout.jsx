...
export default function AppLayout() {
    ...
    const inventoryItemNames = ["torch", "key", "map", "amulet", "knowledge", "potion", "sword", "gold"];
    ...
    function handleSetInventory(item, isGiving = true) {
        if (isGiving) {
            if (inventoryItemNames.includes(item) && !inventory.includes(item)) {
    ...
    return <Container fluid>
        <Row>
            <Col lg={3} style={{ padding: '0px', maxHeight: '100vh', overflowY: 'auto', overflowX: 'none', scrollbarWidth: 'none' }}>
